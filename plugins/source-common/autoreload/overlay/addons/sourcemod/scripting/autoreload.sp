...
#define MAX_PLUGIN_FILENAME_LENGTH 64
#define MAX_WHITELIST_COUNT 16

StringMap g_hMapFiles;
char g_sRoot[PLATFORM_MAX_PATH], g_sPathSeparator[4];
ConVar g_hCvarEnable, g_hCvarUpdateDelay, g_hCvarTrackNew, g_hCvarShowInfo, g_hCvarErrorLines, g_hCvarMsgFlag, g_hCvarWhitelist;
bool g_bEnable, g_bTrackNew, g_bListChanged, g_bLateload, g_bMapStart, g_bSeqStart, g_bNewSeq;
int g_iShowInfo, g_iLenRoot, g_iErrLines, g_iMsgFlags, g_iWhitelistCount;
Handle g_hTimer, g_OnUpdateForward, g_OnReloadForward, g_OnLoadForward, g_OnUnloadForward;
char g_aWhiteListName[MAX_WHITELIST_COUNT][MAX_PLUGIN_FILENAME_LENGTH];
char g_aWhiteListPath[MAX_WHITELIST_COUNT][MAX_PLUGIN_FILENAME_LENGTH];
...
/* Fork by Dragokas
	...
	1.18 (29-Mar-2026)
	 - Added "sm_autoreload_whitelist" Convar - Comma separated list of plugin file names (without .smx part) to track (leave empty - to track all plugins)
	 - Fixed "over-optimized" code caused newly created plugins might have been outside the tracking system.
	
	1.17 (24-Mar-2026)
	 - Fixed missing CloseHandle.
...
public void OnPluginStart()
{
	...
	g_hCvarUpdateDelay = CreateConVar( 	"sm_autoreload_delay", 			"2.0", 	"Delay between updates (in sec.). Set 0 to disable timer at all.", CVAR_FLAGS);
	...
	g_hCvarWhitelist = CreateConVar( 	"sm_autoreload_whitelist", 		"", 	"Comma separated list of plugin file names (without .smx part) to track (leave empty - to track all plugins)", CVAR_FLAGS );
	...
	g_hCvarUpdateDelay.AddChangeHook(OnConVarUpdateTimer);
	g_hCvarWhitelist.AddChangeHook(OnConVarWhitelist);
	...
	g_hMapFiles = new StringMap();
	UpdateWhitelist();
	
	if( !g_bLateload ) // on late load, the list is loaded OnMapStart() automatically
	{
		GetPluginList(g_sRoot, true, true);
		PrintAll(-1, "[AutoReload] Found %i plugins.", g_hMapFiles.Size);
	}
	
	UpdateTimer();
}

Action CmdAutoreload(int client, int argc)
{
...
void OnConVarUpdateTimer(Handle convar, const char[] oldValue, const char[] newValue)
{
...
void OnConVarWhitelist(Handle convar, const char[] oldValue, const char[] newValue)
{
	if( strcmp(oldValue, newValue) != 0 )
	{
		UpdateWhitelist();
...
{
	#define MAX_WHITELIST_SIZE ((MAX_PLUGIN_FILENAME_LENGTH+1) * MAX_WHITELIST_COUNT + 1)
	
	g_iWhitelistCount = 0;
	char[] list = new char[MAX_WHITELIST_SIZE];
	g_hCvarWhitelist.GetString(list, MAX_WHITELIST_SIZE);
	
	if( list[0] )
	{
		g_iWhitelistCount = ExplodeString(list, ",", g_aWhiteListName, MAX_WHITELIST_COUNT, MAX_PLUGIN_FILENAME_LENGTH);
		
		for( int i = 0; i < g_iWhitelistCount; i++ )
		{
			TrimString(g_aWhiteListName[i]);
			StrCat(g_aWhiteListName[i], MAX_PLUGIN_FILENAME_LENGTH, ".smx");
			FormatEx(g_aWhiteListPath[i], sizeof(g_aWhiteListPath[]), "%s%s", g_sRoot, g_aWhiteListName[i]);
			
			PrintToServer("WhiteList entry %d: %s", i, g_aWhiteListName[i]);
			PrintToServer("WhiteList path  %d: %s", i, g_aWhiteListPath[i]);
		}
	}
}
...
{
	// SourceMod updates plugins himself in OnMapStart()
	// So, we need to overwrite all timestamps (.bOverwriteTimestamp = true)
	GetPluginList(g_sRoot, true, true);
	g_bListChanged = true;
	g_bMapStart = true;
	g_bSeqStart = false;
...
void GetPluginList(char[] dirname, bool bOverwriteTimestamp, bool bIsMapStarting)
{
	if( g_iWhitelistCount )
	{
		for( int i = 0; i < g_iWhitelistCount; i++ )
		{
			if( FileExists(g_aWhiteListPath[i], false) )
			{
				PopulatePlugin(g_aWhiteListName[i], g_aWhiteListPath[i], bOverwriteTimestamp, bIsMapStarting);
			}
		}
		return;
	}
	...
	static char 	sPluginFile[PLATFORM_MAX_PATH];
	FileType 		fileType;
	int 			iLenPlugin;
	...
	while( dir.GetNext(sPluginFile, sizeof(sPluginFile), fileType) )
	{
		FormatEx(sPath, sizeof(sPath), "%s%s", dirname, sPluginFile);
		
		switch( fileType )
		{
			case FileType_File:
			{
				iLenPlugin = strlen(sPluginFile);
				
				if( iLenPlugin >= 4 && strcmp(sPluginFile[iLenPlugin - 4], ".smx") == 0 )
				{
					Format(sPluginFile, sizeof(sPluginFile), "%s%s", dirname[g_iLenRoot], sPluginFile); // append prefix in recurse calls
					PopulatePlugin(sPluginFile, sPath, bOverwriteTimestamp, bIsMapStarting);
				}
			}
			case FileType_Directory:
			{
				if( strcmp(sPluginFile, ".") != 0 && strcmp(sPluginFile, "..") != 0 && strcmp(sPluginFile, "disabled") != 0 )
				{
					StrCat(sPath, sizeof(sPath), g_sPathSeparator);
					GetPluginList(sPath, bOverwriteTimestamp, bIsMapStarting);
				}
			}
		}
	}
...
void PopulatePlugin(char[] sPluginFile, char[] sPath, bool bOverwriteTimestamp, bool bIsMapStarting)
{
	int iStamp;
	bool isNewFile = !g_hMapFiles.GetValue(sPluginFile, iStamp);
	
	if( bOverwriteTimestamp || isNewFile )
	{
		iStamp = GetFileStamp(sPath);
		g_hMapFiles.SetValue(sPluginFile, iStamp);
		...
		if( isNewFile )
		{
			...
			if( !bIsMapStarting )
			{
				LoadPlugin(sPluginFile, 0.5);
			}
		}
	}
}

Action Timer_Regeneration(Handle timer)
{
	...
	static char sFilename[MAX_PLUGIN_FILENAME_LENGTH];
	...
	for( int i = 0; i < hSnap.Length; i++ )
	{
		...
		if( iStamp != iStampnew )
		{
			...
			g_hMapFiles.SetValue(sFilename, iStampnew, true);
	...
	if( g_bTrackNew )
	{
		GetPluginList(g_sRoot, false, false);
	}
...
Action Timer_ReloadDelayed(Handle timer, DataPack dp)
{
	...
	FormatEx(sPath, sizeof(sPath), "%s%s", g_sRoot, sFilename);
	g_hMapFiles.SetValue(sFilename, GetFileStamp(sPath), true); // ensure we saved actual stamp info, since copy operation could finish just right now or even still processing
